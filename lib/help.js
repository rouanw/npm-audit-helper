...
module.exports = (config = {}) => {
  ...
  function requireAttention(auditResult) {
    ...
    const reviewActions = actions.filter(({ action, resolves, isMajor }) =>
      (action === 'review' || isMajor)
      && affectsProd(resolves)
    );
  ...
  function highestSeverityOnly(auditResult) {
    const { actions, advisories } = requireAttention(auditResult);
    ...
    return Object.assign({}, auditResult, { actions: highPriorityActions, advisories });
  }

  function getAutoFixCount(auditResult) {
    return auditResult.actions.filter(({ action, resolves, isMajor }) =>
      action !== 'review'
      && !isMajor
      && affectsProd(resolves)
    ).length;
  ...
  function getMostProblematicDependency(auditResult) {
    const { actions } = requireAttention(auditResult);
