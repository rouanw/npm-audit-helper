...
function automaticFixes({ actions }) {
  const autoFixCount =  actions.reduce((total, { action }) => action === 'review' ? 0 : 1, 0);
  return autoFixCount
    ? `run \`npm audit fix\` to automatically fix ${automaticFixes} issues. These should all be non-breaking upgrades, so don't stress.`
    : '';
}
