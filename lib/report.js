...
function autoCount({ actions }) {
  return actions.reduce((total, { action }) => action === 'review' ? 0 : 1, 0);
}
...
module.exports = async function printFullReport (auditResult) {
  ...
  const automaticFixes = autoCount(auditResult);
  const hints = []
    .concat(automaticFixes && `run \`npm audit fix\` to automatically fix ${automaticFixes} issues. These should all be non-breaking upgrades, so don't stress.`)
